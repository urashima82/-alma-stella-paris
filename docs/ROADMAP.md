...
- [ ] Modifier la migration initiale pour inclure les tables `stone` et `product_stone`
- [ ] Recréer l'environnement DDEV et vérifier le schéma
...
- [ ] Entrée menu « Pierres » dans la sidebar admin (section Catalogue)
- [ ] `ProductCrudController` : ajouter champ `AssociationField` pour les pierres
...
- [ ] Template détail (`templates/shop/stone/show.html.twig`) :
...
- [ ] Styles cohérents avec le design system (tokens `alma-*`, typographie Cormorant/Inter)

### 15d — Filtre par pierre dans la boutique

- [ ] Ajouter une section « Pierres » dans la sidebar du catalogue (sous les catégories)
...
- [ ] Route : `/shop?stone={slug}` / `/boutique?stone={slug}` (query parameter)
  - Compatible avec le filtre catégorie : `/shop/bracelets?stone=onyx`
- [ ] `CatalogController` : ajouter la logique de filtrage par pierre
- [ ] `ProductRepository` : méthode de requête filtrée par pierre(s)
- [ ] État actif dans la sidebar (pierre sélectionnée mise en surbrillance)
- [ ] Mobile : section pierres intégrée dans le drawer « Filtrer » existant

### 15e — Intégration fiche produit

- [ ] Sur la page produit (`show.html.twig`) : afficher les pierres du produit
...
- [ ] Tooltip ou texte court sous le badge avec la vertu principale

### 15f — Navigation & SEO

- [ ] Lien « Nos pierres » / « Our stones » dans le footer (section « Découvrir »)
- [ ] Meta title/description dynamiques sur les pages index et détail
- [ ] Breadcrumb `BreadcrumbList` structured data sur les pages guide
- [ ] `hreflang` alternates sur toutes les pages pierre
- [ ] Ajouter les pages pierres au sitemap XML
- [ ] Open Graph tags sur les pages détail

### 15g — DataFixtures

- [ ] Charger les pierres depuis `docs/STONES.md` (ou tableau hardcodé dans les fixtures)
- [ ] Associer les pierres aux produits existants dans les fixtures
...
- [ ] Vérifier que les fixtures se chargent sans erreur

### 15h — Documentation

- [ ] Mettre à jour `docs/ARCHITECTURE.md` (nouvelle entité, contrôleur, routes)
- [ ] Mettre à jour les traductions (`messages.en.yaml`, `messages.fr.yaml`)
